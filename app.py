...
def get_prices(product_name, country_code):
    ...
    # Debugging: Check if the response contains shopping results
    if 'shopping_results' not in results:
        st.write(f"Error or no shopping results for {product_name} in {country_code}: {results}")
        return []
    ...
    if products:
        ...
        for product in products:
            for region in selected_regions:
                st.write(f"Fetching prices for {product} in {region}...")  # Debugging message
